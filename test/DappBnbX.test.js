...
describe('Contracts', () => {
  ...
  beforeEach(async () => {
    ...
    contract = await ethers.deployContract('DappBanX', [taxPercent, securityFee])
  ...
  //     await contract
  //       .connect(owner)
  //       .updateApartment(id, newName, description, location, 'img', rooms, toWei(newPrice))
  ...
  describe('Booking', () => {
    describe('Success', () => {
      ...
      // it('Should confirm apartment fund claim', async () => {
      //   result = await contract.getBooking(id, bookingId)
      //   expect(result.abandoned).to.be.equal(false)

      //   await contract.connect(tenant1).refundBooking(id, bookingId)

      //   result = await contract.getBooking(id, bookingId)
      //   expect(result.abandoned).to.be.equal(true)
      // })
